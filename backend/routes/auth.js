...
router.post("/signup", isLoggedOut, (req, res) => {
  ...
  if (!username) {
    return res
      .status(400)
      .json("auth/signup", { errorMessage: "Please provide your username." });
  }
...
router.post("/login", isLoggedOut, (req, res, next) => {
  ...
  if (!username) {
    return res
      .status(400)
      .json("auth/login", { errorMessage: "Please provide your username." });
  }
  ...
  User.findOne({ username })
    .then((user) => {
      ...
      if (!user) {
        return res
          .status(400)
          .json("auth/login", { errorMessage: "Wrong credentials." });
      }
...
router.get("/logout", isLoggedIn, (req, res) => {
  req.session.destroy((err) => {
    if (err) {
      return res
        .status(500)
        .json("auth/logout", { errorMessage: err.message });
    }
    res.redirect("/");
  });
});
